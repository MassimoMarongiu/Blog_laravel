        ...
        <div class="d-flex align-items-center justify-content-between">
            <div class="d-flex align-items-center">
                <img style="width:50px" class="me-2 avatar-sm rounded-circle"
                    src="{{$message->user->getImageURL()}}" alt="{{$message->user->name}}">
                    {{-- {{$message->$user->getImageURL()}} --}}
                <div>
                    <h5 class="card-title mb-0"><a href="{{route('users.show',$message->user_id)}}"> {{$message->user->name}}
                        </a></h5>
                </div>
            ...
            <div class="ms-3 col">
                <form action="{{ route('messages.destroy', $message->id) }}" method="POST">
                    ...
                    <button class="btn btn-success btn-sm mb-1 ms-1 float-end">
                        <a href="{{ route('messages.show', $message->id) }}">Dettagli</a>
                    </button>
                    <button class="btn btn-danger btn-sm float-end ms-1"> Delete </button>
